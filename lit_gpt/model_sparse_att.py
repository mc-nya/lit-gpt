...
class GPT(nn.Module):
    ...
    def get_block_buffers_by_name(self, name):
        return torch.stack([getattr(block.attn, name) for block in self.transformer.h])
    def parameters_by_names(self, names):
        """ This function is used to get the parameters for optimizer.
        It works as a special case of `parameters` function.
        So we can only optimize the desired parameters and freeze the pre-trained ones.
        It search the parameters in block.attn, and return the parameters in the list.
        """
        for name, param in self.named_parameters():
            for n in names:
                if n in name:
                    yield param
    def parameter_exclude_names(self, names):
        for name, param in self.named_parameters():
            for n in names:
                if n not in name:
                    yield param
...
class CausalSelfAttention(nn.Module):
    ...
    def top_k_sum(self, att: torch.Tensor, k_ratio=0.1) -> torch.Tensor:
        # att B, nh, T, T
        _, _, T, _ = att.size()
        if isinstance(k_ratio, int):
            top_values, _ = torch.topk(att, k=k_ratio, dim=-1)
            return top_values.sum(dim=-1)
        elif isinstance(k_ratio, float):
            seq_length = torch.arange(T, device=att.device).long()+1
            n_top = (seq_length * k_ratio).clamp(min=1).long()
            def row_top_k(att_row, n_top):
                top_values, _ = torch.topk(att_row, k=n_top, dim=-1)
                top_sums = top_values.sum(dim=-1)
                return top_sums
            top_sums = torch.stack([row_top_k(att_row, n) for att_row, n in zip(torch.permute(att, [2, 0, 1, 3]), n_top)]).permute([1, 2, 0])
            return top_sums
        else:
            raise ValueError(f"Unsupported type for k_ratio: {type(k_ratio)}")
    ...
    def scaled_dot_product_attention(
        ...
        if not hasattr(self, "row_length_buffer"):
            row_length = torch.arange(1, self.config.block_size+1, device=self.mask_cache.device).float()
        ...
        # for each row in attention matrix, apply row = row * alpha*(row_length^beta)
        # att = att  * (self.row_length_buffer ** (2. * torch.sigmoid(self.beta)-1.))
        y = att @ v
        return y.transpose(1, 2)
