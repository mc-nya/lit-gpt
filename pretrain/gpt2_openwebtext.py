...
from lit_gpt.gpt_2 import GPTConfig as Config
from lit_gpt.gpt_2 import GPT, Block
...
name = "124M-lit-mistral"
config_file = "configs/gpt_2_124M_mistral.json"
...
def train(fabric: L.Fabric, state: dict, train_dataloader: DataLoader, val_dataloader: DataLoader) -> None:
    ...
    #     # if iter_num % log_interval == 0:
    #     if not is_accumulating and state["step_count"] % log_interval == 0:
    #         loss_item = loss.item()  # expensive device-to-host synchronization
    #         t1 = time.perf_counter()
    #         throughput.update(
    #             time=t1 - total_t0,
    ...
    #         fabric.log_dict(metrics = {"running/loss": loss_item, "running/lr": lr, 
    ...
    #         val_loss = validate(fabric, model, val_dataloader, max_iters=eval_iters)
    #         t1 = time.perf_counter() - t0
    #         fabric.log_dict(metrics = {"val/loss": val_loss.item(), "val/time": t1 * 1000,
    #                                     "step": state['step_count'],
    #                                     "running/valtime": t1 * 1000})
    #         fabric.print(f"step {iter_num}: val loss {val_loss.item():.4f}, val time: {t1 * 1000:.2f}ms")
    #         fabric.barrier()
