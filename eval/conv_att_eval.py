...
from generate.base_nokv import generate
...
class EvalHarnessBase(BaseLM):
    ...
    def __init__(self, fabric: L.Fabric, model, tokenizer: Tokenizer,
                 batch_size: int):
        super().__init__()
        self.fabric = fabric
        self.model = model
    ...
    @torch.inference_mode()
    def run_eval(self, eval_tasks: List[str], num_fewshot: int,
                 limit: Optional[int], bootstrap_iters: int,
                 no_cache: bool) -> Dict:
...
def run_eval_harness(
    ...
    # Here, we add additional print for the conv weights:
    ...
    results = eval_harness.run_eval(eval_tasks, num_fewshot, limit,
                                    bootstrap_iters, no_cache)
