...
# if sorted_attention[0,0,0] >1:
#     sorted_attention /= sorted_attention[0,0,0]
os.makedirs("out/fig/attn", exist_ok=True)

sorted_attention = 1-np.cumsum(sorted_attention, axis=-1)
sorted_attention[np.where(sorted_attention < 1e-7)] = 1e-7
...
for layer, attention in enumerate(sorted_attention):
    ...
    specific_tokens = [(total_tokens//4)-1 , (total_tokens//2)-1, (3*total_tokens//4)-1, total_tokens-2]
    specific_colors = ['red', 'green', 'blue', 'orange']  # Colors for the specific lines
    ...
    for t in specific_tokens:
        ...
        # find the first index where the attention is smaller than threshold
        prob_token.append(np.where(token_attention < threshold)[0][0])
    ...
    print(f"Layer {layer}: {prob_token}")
    print(f"propotion: {np.round(np.array(prob_token)/prob_token[-1], 4)}")
